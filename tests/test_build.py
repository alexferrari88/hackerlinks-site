...
class BuildTests(unittest.TestCase):
    ...
    def test_build_public_site_exports_core_routes_and_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            corrections_html = (dist_root / "corrections" / "index.html").read_text()
            ...
            self.assertIn("Search the archive", index_html)
            self.assertIn("Browse latest issue", index_html)
            self.assertIn("Get the daily finds", index_html)
            self.assertLess(index_html.index("Search the archive"), index_html.index("Get the daily finds"))
            self.assertLess(index_html.index("Browse latest issue"), index_html.index("Get the daily finds"))
            self.assertEqual(index_html.count('class="board-row"'), 5)
            self.assertIn("View all 6 finds in this issue", index_html)
            self.assertNotIn("Fresh finds", index_html)
            self.assertIn("Keeps coming up", index_html)
            self.assertIn("Commenters recommended Tailscale as the easiest safe remote-access layer", index_html)
            self.assertNotIn("Cloudflare CLI", index_html)
            ...
            self.assertIn("A day-by-day trail of what HN uncovered.", issues_html)
            archive_html = (dist_root / "archive" / "index.html").read_text()
            self.assertIn("What was that thing someone mentioned on HN?", archive_html)
            self.assertIn("Search the archive", archive_html)
            self.assertIn("About", about_html)
            self.assertIn("independent and unaffiliated with Hacker News or Y Combinator", about_html)
            self.assertIn("operated and edited by the maintainer", about_html)
            self.assertIn("@alexferrari88", about_html)
            self.assertIn("Methodology", methodology_html)
            self.assertIn("AI-assisted", methodology_html)
            self.assertIn("original comments are authoritative", methodology_html)
            self.assertIn("Corrections and contact", corrections_html)
            self.assertIn("github.com/alexferrari88/hackerlinks-site/issues", corrections_html)
            self.assertIn("no paid placements", corrections_html)
            self.assertIn("/corrections/", index_html)
            self.assertIn("Daily issue / 2026-04-14", issue_html)
            self.assertIn("How these finds were selected", issue_html)
            self.assertIn("Cloudflare CLI", issue_html)
            self.assertIn("DaVinci Resolve", item_html)
            self.assertIn("Archive ID", item_html)
            self.assertIn("Why it caught our attention", item_html)
            self.assertIn("How this record is sourced", item_html)
    ...
    def test_build_public_site_preserves_item_ordering_and_issue_navigation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Older issue", issue_2_html)
            self.assertIn("/issues/2026-04-14", issue_2_html)
            self.assertIn("Matching finds", archive_html)
            self.assertIn("Seen more than once", archive_html)
